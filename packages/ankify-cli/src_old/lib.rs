...
impl Ankify {
    ...
    async fn render_card(&self, card: &Card) -> Result<RenderedCard> {
        // Since we now render cards from the source file, we need to get the source file
        // and render all cards together. For now, we'll render this single card.
        let cards = vec![card.clone()];
        let results = self.renderer
            .render_cards_from_source(&card.source_file, &cards, self.config.render_format)
            .await?;

        results.into_iter().next()
            .ok_or_else(|| Error::render("No rendered card returned".to_string()))
    }
}
