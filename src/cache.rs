...
impl Cache {
    ...
    ///
    /// 1. If neither hash nor label match, create a new card
    /// 2. If label matches but hash doesn't, update the card
    /// 3. If label doesn't match but hash matches exactly one entry, update the label
    /// 4. If both label and hash match, skip (no changes needed)
    /// 5. If cache contains entries not in the current cards, delete them
    ///
    /// # Arguments
    ///
    ...
    pub fn plan_operations(&self, cards: &[Card]) -> Result<Vec<Operation>> {
        ...
        for card in cards {
            ...
            if let Some(entry) = self.entries.get(&card.label) {
                // Label matches
                if entry.hash == hash {
                    // Both label and hash match - no changes needed
                    operations.push(Operation::Skip { card: card.clone() });
                } else {
                    // Label matches but hash doesn't - update needed
                ...
                // Label doesn't match - check if hash matches any other entry
                ...
                match matching_entries.len() {
                    0 => {
                        // No matching hash - create new card
                        operations.push(Operation::Create { card: card.clone() });
                    }
                    1 => {
                        // Hash matches exactly one entry - this is a label change
                        let matching_entry = matching_entries[0];
                        operations.push(Operation::Update {
                            card: card.clone(),
                            anki_id: matching_entry.id.clone(),
                        });

                        tracing::info!(
                            "Detected label change: '{}' -> '{}'",
                            matching_entry.label,
                            card.label
                        );
                    }
                    _ => {
                        // Hash matches multiple entries - ambiguous, create new
                        tracing::warn!(
                            "Hash collision detected for card '{}', creating new card",
                            card.label
                        );
                        operations.push(Operation::Create { card: card.clone() });
                    }
